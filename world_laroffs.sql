...
WITH DELETE_CTE AS (
  ...
  HAVING COUNT(*) > 1 -- This identifies rows with duplicates
)
...
INNER JOIN DELETE_CTE d 
ON 
  ls.company = d.company
  AND ls.location = d.location
  AND -- Add all other columns used for identifying duplicates
  ls.industry = d.industry
